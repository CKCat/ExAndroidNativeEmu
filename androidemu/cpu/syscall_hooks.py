...
class SyscallHooks:
    ...
    def _handle_gettimeofday(self, uc, tv, tz):
        ...
        if tz != 0:
            ...
            uc.mem_write(
                tz + 0, int(-120).to_bytes(4, byteorder="little")
    ...
    def __sysinfo(self, mu, info_ptr):
        ...
        logger.warning(
            "syscall sysinfo buf 0x%08X return fixed value" % (info_ptr)
        )
        return 0
    ...
    def _handle_prctl(self, mu, option, arg2, arg3, arg4, arg5):
        ...
        if option == PR_SET_VMA:
            ...
            mu.mem_write(
                arg2, int(0).to_bytes(self.__ptr_sz, byteorder="little")
            )
            return 0
            ...
            raise NotImplementedError(
                "Unsupported prctl option %d (0x%x)" % (option, option)
            )
    ...
    def __uname(self, mu, buf):
        ...
        memory_helpers.write_utf8(
            mu, buf + 195, "#1 SMP PREEMPT Mon Nov 2 20:10:58 UTC 2015"
        )
    ...
    def _handle_futex(self, mu, uaddr, op, val, timeout_ptr, uaddr2, val3):
        ...
        if cmd == FUTEX_WAIT or cmd == FUTEX_WAIT_BITSET:
            ...
            assert val <= 0x7FFFFFFF, (
                "futex wake val=0x%08X bigger than int max!!!" % val
            )
            nwake = 0
    ...
    def _handle_clock_gettime(self, mu, clk_id, tp_ptr):
        ...
        if clk_id == CLOCK_REALTIME:
            ...
            mu.mem_write(
                tp_ptr + 0,
                int(clock_real).to_bytes(self.__ptr_sz, byteorder="little"),
            )
            mu.mem_write(
                tp_ptr + self.__ptr_sz,
                int(0).to_bytes(self.__ptr_sz, byteorder="little"),
            )
            return 0
            ...
            if OVERRIDE_CLOCK:
                mu.mem_write(
                    ...
                    int(OVERRIDE_CLOCK_TIME).to_bytes(
                        self.__ptr_sz, byteorder="little"
                    ),
                )
                mu.mem_write(
                    tp_ptr + self.__ptr_sz,
            ...
            raise NotImplementedError(
                "Unsupported clk_id: %d (%x)" % (clk_id, clk_id)
            )
    ...
    def _ARM_set_tls(self, mu, tls_ptr):
        assert self.__emu.get_arch() == emu_const.ARCH_ARM32, (
    ...
    def _nanosleep(self, mu, req, rem):
        ...
        ms = req_tv_sec * 1000 + req_tv_nsec / 1000000
        logger.debug(f"nanosleep sleep {ms:.3f} ms.")
        sch = self.__emu.get_schduler()
        sch.sleep(ms)
        return 0
