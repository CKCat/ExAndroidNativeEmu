...
class ELFReader:
    ...
    def __st_name_to_name(self, st_name):
        assert st_name < self.__dyn_str_sz, (
        ...
        with open(filename, "rb") as f:
            ...
            while True:
                ...
                #
                    ...
                    hash_heads = f.read(8)
                    f.seek(n, 0)
                    self.__nbucket, self.__nchain = struct.unpack(
                        "<II", hash_heads
                    )
                ...
                #
                    ...
                    f.seek(ori, 0)
                    gnu_nbucket_, symoffset, gnu_maskwords_, gnu_shift2_ = (
            ...
            for i in range(0, nsymbol):
                ...
                try:
                    ...
                    logger(
                        f"warning can not decode sym index {i} at off 0x{st_name:08x} skip"
                    )
    ...
    def __write_soinfo32(self, mu, load_base, load_bias, info_base):
        ...
        mu.mem_write(
            info_base + 128,
            int(load_base + self.__phoff).to_bytes(4, byteorder="little"),
        )
        # phnum
        mu.mem_write(
            info_base + 132,
            int(self.__phdr_num).to_bytes(4, byteorder="little"),
        )
        # entry
        ...
        # size
        mu.mem_write(
            info_base + 144, int(self.__sz).to_bytes(4, byteorder="little")
        )
        # unused1
        ...
        mu.mem_write(
            info_base + 152,
            int(load_base + self.__dyn_addr).to_bytes(4, byteorder="little"),
        )
        # unused2
        ...
        mu.mem_write(
            info_base + 172,
            int(load_base + self.__dyn_str_addr).to_bytes(
                4, byteorder="little"
            ),
        )
        # symtab
        ...
        # nbucket
        mu.mem_write(
            info_base + 180, int(self.__nbucket).to_bytes(4, byteorder="little")
        )
        # nchain
        mu.mem_write(
            info_base + 184, int(self.__nchain).to_bytes(4, byteorder="little")
        )

        # bucket
        mu.mem_write(
            info_base + 188,
            int(load_base + self.__bucket_addr).to_bytes(4, byteorder="little"),
        )
        # nchain
        mu.mem_write(
            info_base + 192,
            int(load_base + self.__chain_addr).to_bytes(4, byteorder="little"),
        )

        # plt_got
        mu.mem_write(
            info_base + 196,
            int(load_base + self.__plt_got_addr).to_bytes(
                4, byteorder="little"
            ),
        )
        ...
        mu.mem_write(
            info_base + 200,
            int(load_base + self.__pltrel_addr).to_bytes(4, byteorder="little"),
        )
        # plt_rel_count
        mu.mem_write(
            info_base + 204,
            int(self.__pltrel_count).to_bytes(4, byteorder="little"),
        )

        # rel
        mu.mem_write(
            info_base + 208,
            int(load_base + self.__rel_addr).to_bytes(4, byteorder="little"),
        )
        # rel_count
        mu.mem_write(
            info_base + 212,
            int(self.__rel_count).to_bytes(4, byteorder="little"),
        )
        ...
        mu.mem_write(
            info_base + 224,
            int(load_base + self.__init_array_addr).to_bytes(
                4, byteorder="little"
            ),
        )
        # init_array_count
        mu.mem_write(
            info_base + 228,
            int(self.__init_array_size / 4).to_bytes(4, byteorder="little"),
        )
        ...
        mu.mem_write(
            info_base + 240,
            int(load_base + self.__init_addr).to_bytes(4, byteorder="little"),
        )
        # fini_func
        ...
        # Elf32_Addr load_bias
        load_bias = load_base - (
            self.__loads[0]["p_vaddr"] - self.__loads[0]["p_offset"]
        )
        mu.mem_write(
    ...
    def __write_soinfo64(self, mu, load_base, load_bias, info_base):
        ...
        mu.mem_write(
            info_base + off,
            int(load_base + self.__phoff).to_bytes(8, byteorder="little"),
        )
        off += 8
        # phnum
        mu.mem_write(
            info_base + off,
            int(self.__phdr_num).to_bytes(8, byteorder="little"),
        )
        off += 8
        ...
        # size
        mu.mem_write(
            info_base + off, int(self.__sz).to_bytes(8, byteorder="little")
        )
        off += 8
        ...
        mu.mem_write(
            info_base + off,
            int(load_base + self.__dyn_addr).to_bytes(8, byteorder="little"),
        )
        off += 8
        ...
        # nbucket
        mu.mem_write(
            info_base + off, int(self.__nbucket).to_bytes(4, byteorder="little")
        )
        off += 8
        # nchain
        mu.mem_write(
            info_base + off, int(self.__nchain).to_bytes(4, byteorder="little")
        )
        off += 8
        ...
        mu.mem_write(
            info_base + off,
            int(load_base + self.__pltrel_addr).to_bytes(8, byteorder="little"),
        )
        off += 8
        # plt_rela_count
        mu.mem_write(
            info_base + off,
            int(self.__pltrel_count).to_bytes(8, byteorder="little"),
        )
        off += 8
        ...
        mu.mem_write(
            info_base + off,
            int(load_base + self.__rel_addr).to_bytes(8, byteorder="little"),
        )
        off += 8
        ...
        mu.mem_write(
            info_base + off,
            int(self.__rel_count).to_bytes(8, byteorder="little"),
        )
        off += 8
        ...
        mu.mem_write(
            info_base + off,
            int(self.__init_array_size / 8).to_bytes(8, byteorder="little"),
        )
        off += 8
        ...
        mu.mem_write(
            info_base + off,
            int(load_base + self.__init_addr).to_bytes(8, byteorder="little"),
        )
        off += 8
        # fini_func
        mu.mem_write(info_base + off, int(0).to_bytes(8, byteorder="little"))
